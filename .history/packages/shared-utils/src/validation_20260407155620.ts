...
export function validateOriginCountry(
  value: string,
): ValidationSuccess<string> | ValidationFailure {
  return validateLocationField(value, "Country of origin");
}
...
export function validateRequiredUserProfile(
  ...
  const originCountryResult = validateOriginCountry(input.originCountry);

  if (
    !fullNameResult.ok ||
    !universityCodeResult.ok ||
    !nationalityResult.ok ||
    !originCountryResult.ok
  ) {
    ...
    if (!originCountryResult.ok) {
      fieldErrors.originCountry = originCountryResult.error;
    }
    ...
    return {
      ...
      fieldErrors,
      message:
        "Profile completion requires valid full name, university code, nationality, and country of origin.",
    };
  }
  ...
  return {
    ...
    value: {
      ...
      originCountry: originCountryResult.value,
...
export function evaluateProfileCompletion(input: {
  ...
  const originCountryResult = validateOriginCountry(
    String(input.originCountry || ""),
  );
  ...
  return {
    ...
    normalizedOriginCountry: originCountryResult.ok ? originCountryResult.value : null,
