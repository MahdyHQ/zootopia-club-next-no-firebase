...
/* Generated assessment outputs and export artifacts are cached in owner-scoped Storage so
  every download remains server-authenticated and path-isolated. Export artifacts follow
  the export-scope retention window, while canonical result artifacts follow result-scope.
  Future agents should add new artifact kinds through this helper
...
export async function persistAssessmentResultArtifact(
  ...
  const resultFileName = buildAssessmentGeneratedFileName({
    generation,
    extension: "json",
  });
