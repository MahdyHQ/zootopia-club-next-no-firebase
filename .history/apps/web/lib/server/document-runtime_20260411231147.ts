...
function resolveWorkspaceExpiryTimestamp(input: {
  ...
  const retentionExpiry = getRetentionExpiryTimestamp(input.createdAt, "uploads");
  const retentionExpiryMs = retentionExpiry ? Date.parse(retentionExpiry) : Number.NaN;

  if (Number.isFinite(workspaceExpiryMs) && Number.isFinite(retentionExpiryMs)) {
    return new Date(Math.min(workspaceExpiryMs, retentionExpiryMs)).toISOString();
  }
  ...
  // Return undefined when retention mode is "none" (no expiry)
  return retentionExpiry ?? undefined;
}
...
>) {
  ...
  try {
    /* STORAGE READ OWNERSHIP GUARD:
       
       Ownership is validated TWICE:
       1. Metadata layer: Route Handler verified record.ownerUid === session.uid before calling this
       2. Path layer: assertOwnerScopedStoragePath() re-confirms path starts with ownerUid/
       
       This three-part check (metadata lookup + metadata validation + path assertion)
       means even corrupted/leaked storage paths cannot be exploited.
    */
...
>) {
  ...
  try {
    /* STORAGE DELETE OWNERSHIP GUARD (Same Model as Read):
       
       Delete operations are protected by the same two-layer ownership check.
       Route Handler verified session.uid ownership before calling this.
       Path assertion adds a final guardrail against accidental scope violations.
    */
    const storagePath = assertOwnerScopedStoragePath(record.storagePath, record.ownerUid, [
      "documents",
    ]);
