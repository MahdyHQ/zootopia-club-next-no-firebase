...
import {
  getZootopiaDatabase,
  /** @deprecated Legacy alias for backward compatibility — points to Supabase Postgres, not Firebase */
 ...
 * Returns true when Supabase admin runtime is available and database is configured.
 * Falls back to in-memory store for dev/degraded runtime.
 */
...
/**
 * Legacy alias for backward compatibility with existing call sites.
 * @deprecated Use `shouldUseDatabase()` instead.
 */
const shouldUseFirestore = shouldUseDatabase;
    ...
    await getZootopiaFirestore().runTransaction(async (transaction) => {
      ...
      if (!isAdmin && account.assessmentAccess === "disabled") {
        console.warn("[assessment-credit-commit]", {
          event: "assessment-access-disabled-firestore",
          uid: input.user.uid,
          role: input.user.role,
