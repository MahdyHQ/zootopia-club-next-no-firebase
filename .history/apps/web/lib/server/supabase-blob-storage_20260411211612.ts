 ...
 * Paginates internally to collect all matching objects.
 */
  ...
  try {
    ...
    while (true) {
      ...
      for (const item of data) {
        // item.name is the leaf name within the prefix; reconstruct full path.
        const fullPath = prefix ? `${prefix}/${item.name}` : item.name;
        if (item.metadata?.mimeType || item.name) {
          // Only include files (directories have no extension and are implicit).
