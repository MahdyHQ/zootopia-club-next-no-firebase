...
export function getDecodedSignInProvider(
  decodedToken: Pick<DecodedAuthToken, "app_metadata" | "auth_provider" | "firebase">,
) {
  ...
  const legacyProvider =
    typeof legacyProviderClaims?.sign_in_provider === "string"
      ? legacyProviderClaims.sign_in_provider
      : null;

  return legacyProvider ? normalizeSupabaseProviderForAuthChecks(legacyProvider) : null;
}
