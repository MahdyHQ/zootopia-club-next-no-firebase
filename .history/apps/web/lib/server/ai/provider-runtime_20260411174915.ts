...
function getRequiredEnv(name: "GOOGLE_AI_API_KEY" | "DASHSCOPE_API_KEY"): string | null {
...
function resolveProviderModelId(model: ReturnType<typeof getModelById>) {
  const configuredModel = normalizeRuntimeValue(process.env[model.runtimeEnvKey]);
  return configuredModel || model.id;
}
...
export function resolveProviderRuntime(modelId: string) {
  ...
  if (model.provider === "google") {
    ...
    const missingEnvKeys = apiKey ? [] : ["GOOGLE_AI_API_KEY"];
  ...
  const missingEnvKeys = [
    !apiKey ? "DASHSCOPE_API_KEY" : null,
    !baseUrl ? QWEN_BASE_URL_ENV_KEYS.join("|") : null,
