...
type QwenOpenAiCompatibleChatCompletionRequest = {
  model: string;
  temperature: number;
  response_format: {
    type: "json_object";
...
// Assessment Qwen execution uses one OpenAI-compatible client wrapper so all request
// formatting and endpoint composition stay centralized and deterministic.
function createQwenOpenAiCompatibleClient(input: {
  baseUrl: string;
  apiKey: string;
}) {
  const chatCompletionsEndpoint = `${input.baseUrl}/chat/completions`;
...
async function executeQwenAssessmentModel(input: {
  ...
  const qwenClient = createQwenOpenAiCompatibleClient({
