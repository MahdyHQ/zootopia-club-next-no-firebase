...
export type AssessmentExecutionErrorLayer =
  | "request"
  | "provider-runtime"
  | "provider-execution"
  | "provider-response";

const ASSESSMENT_CREATE_ROUTE = "/api/assessment" as const;
const ASSESSMENT_CREATE_FLOW = "assessment-create" as const;

export type AssessmentExecutionErrorContext = {
  layer: AssessmentExecutionErrorLayer;
  subsystem: "assessment-runtime" | "google-runtime" | "qwen-runtime" | "provider-http" | "provider-parser";
  operation: string;
  route: typeof ASSESSMENT_CREATE_ROUTE;
  flow: typeof ASSESSMENT_CREATE_FLOW;
  modelId?: string;
...
type AssessmentExecutionErrorContextInput = Omit<
  AssessmentExecutionErrorContext,
  "route" | "flow"
>;

type AssessmentExecutionContextBase = {
  modelId: string;
  canonicalModelId: string;
  provider: "google" | "qwen";
  providerModelId: string;
...
export class AssessmentExecutionError extends Error {
  constructor(
    ...
    public readonly context: AssessmentExecutionErrorContext,
  ) {
    super(message);
...
function createAssessmentExecutionError(input: {
  code: AssessmentExecutionErrorCode;
  message: string;
  status?: number;
  context: AssessmentExecutionErrorContextInput;
}) {
  return new AssessmentExecutionError(
    ...
    input.status ?? 502,
    {
      route: ASSESSMENT_CREATE_ROUTE,
      flow: ASSESSMENT_CREATE_FLOW,
      ...input.context,
    },
  );
}
...
function buildNormalizedAssessmentQuestions(input: {
  ...
  if (providerQuestions.length === 0) {
    throw createAssessmentExecutionError({
      ...
      message: "The selected model returned an invalid assessment format.",
      status: 502,
      context: {
        layer: "provider-response",
        subsystem: "provider-parser",
        operation: "validate-provider-questions-array",
...
function buildProviderTimeoutError(input: {
  providerLabel: "Google" | "Qwen";
  contextBase: AssessmentExecutionContextBase;
  subsystem: "google-runtime" | "qwen-runtime" | "provider-http";
  operation: string;
}) {
  return createAssessmentExecutionError({
    code: "ASSESSMENT_PROVIDER_TIMEOUT",
    message: `The selected ${input.providerLabel} model did not respond before the request timeout.`,
    status: 504,
    context: {
...
function buildProviderExecutionError(input: {
  ...
  const providerHttpContext = {
    ...input.contextBase,
    layer: "provider-execution" as const,
    subsystem: "provider-http" as const,
    operation: "classify-provider-http-error",
    upstreamStatus: snapshot.httpStatus,
    upstreamCode: snapshot.providerCode || undefined,
    upstreamType: snapshot.providerType || undefined,
  };
  ...
  if (
    ...
    return buildProviderTimeoutError({
      ...
      contextBase: input.contextBase,
      subsystem: "provider-http",
      operation: "classify-provider-http-timeout",
    });
  }
  ...
  return createAssessmentExecutionError({
    ...
    message: `The selected ${input.providerLabel} model could not complete the assessment request right now.`,
    status: 502,
    context: providerHttpContext,
  });
}
...
async function executeGoogleAssessmentModel(input: {
  ...
  if (input.runtime.provider !== "google") {
    throw createAssessmentExecutionError({
      code: "ASSESSMENT_PROVIDER_ROUTE_MISMATCH",
      message: "The selected model was routed to the wrong provider runtime.",
      status: 500,
      context: {
        layer: "provider-runtime",
        subsystem: "google-runtime",
        operation: "validate-google-runtime-provider",
        ...contextBase,
      },
    });
  }
  ...
  if (!input.runtime.apiKey) {
    throw createAssessmentExecutionError({
      code: "ASSESSMENT_PROVIDER_API_KEY_MISSING",
      message: "Google assessment runtime is missing GOOGLE_AI_API_KEY on the server.",
      status: 503,
      context: {
        layer: "provider-runtime",
        subsystem: "google-runtime",
        operation: "validate-google-runtime-api-key",
        ...contextBase,
      },
    });
  }
  ...
  try {
    ...
    if (isAbortError(error)) {
      throw buildProviderTimeoutError({
  ...
  try {
    ...
    throw createAssessmentExecutionError({
      code: "ASSESSMENT_PROVIDER_RESPONSE_INVALID",
      message: "The selected Google model returned an unreadable response.",
      status: 502,
      context: {
        layer: "provider-response",
        subsystem: "provider-parser",
        operation: "parse-google-json-response",
        ...contextBase,
      },
    });
  }
  ...
  if (!responseText) {
    throw createAssessmentExecutionError({
      code: "ASSESSMENT_PROVIDER_RESPONSE_INVALID",
      message: "The selected Google model returned an empty assessment response.",
      status: 502,
      context: {
        layer: "provider-response",
        subsystem: "provider-parser",
        operation: "extract-google-response-text",
        ...contextBase,
      },
    });
  }
  ...
  try {
    ...
    throw createAssessmentExecutionError({
      code: "ASSESSMENT_PROVIDER_RESPONSE_INVALID",
      message: "The selected Google model returned an invalid assessment format.",
      status: 502,
      context: {
        layer: "provider-response",
        subsystem: "provider-parser",
        operation: "parse-google-assessment-payload",
        ...contextBase,
      },
    });
  }
...
async function executeQwenAssessmentModel(input: {
  ...
  if (input.runtime.provider !== "qwen") {
    throw createAssessmentExecutionError({
      ...
      context: {
        ...
        operation: "validate-qwen-runtime-provider",
        ...contextBase,
      },
    });
  }
  ...
  if (!input.runtime.apiKey) {
    throw createAssessmentExecutionError({
      code: "ASSESSMENT_PROVIDER_API_KEY_MISSING",
      message: "Qwen assessment runtime is missing DASHSCOPE_API_KEY on the server.",
      status: 503,
      context: {
        layer: "provider-runtime",
        subsystem: "qwen-runtime",
        operation: "validate-qwen-runtime-api-key",
        ...contextBase,
      },
    });
  }

  if (!input.runtime.baseUrl) {
    throw createAssessmentExecutionError({
      code: "ASSESSMENT_PROVIDER_BASE_URL_MISSING",
      message: `Qwen assessment runtime is missing a DashScope-compatible base URL. Configure one of: ${QWEN_BASE_URL_ENV_HINT}.`,
      status: 503,
      context: {
        layer: "provider-runtime",
        subsystem: "qwen-runtime",
        operation: "validate-qwen-runtime-base-url",
        ...contextBase,
      },
    });
  }
  ...
  try {
    response = await qwenClient.createChatCompletion(
      {
  ...
  let payload: QwenOpenAiCompatibleChatCompletionResponse;

  try {
    payload = (await response.json()) as QwenOpenAiCompatibleChatCompletionResponse;
  } catch {
    throw createAssessmentExecutionError({
      code: "ASSESSMENT_PROVIDER_RESPONSE_INVALID",
      message: "The selected Qwen model returned an unreadable response.",
      status: 502,
      context: {
        layer: "provider-response",
        subsystem: "provider-parser",
        operation: "parse-qwen-json-response",
        ...contextBase,
      },
    });
  }
  ...
  if (!responseText) {
    throw createAssessmentExecutionError({
      code: "ASSESSMENT_PROVIDER_RESPONSE_INVALID",
      message: "The selected Qwen model returned an empty assessment response.",
      status: 502,
      context: {
        layer: "provider-response",
        subsystem: "provider-parser",
        operation: "extract-qwen-response-text",
        ...contextBase,
      },
    });
  }
  ...
  try {
    ...
    throw createAssessmentExecutionError({
      code: "ASSESSMENT_PROVIDER_RESPONSE_INVALID",
      message: "The selected Qwen model returned an invalid assessment format.",
      status: 502,
      context: {
        layer: "provider-response",
        subsystem: "provider-parser",
        operation: "parse-qwen-assessment-payload",
        ...contextBase,
      },
    });
  }
...
async function executeAssessmentModel(input: {
  ...
  switch (input.model.id) {
    ...
    default:
      throw createAssessmentExecutionError({
        code: "ASSESSMENT_MODEL_UNSUPPORTED",
        message: "The selected assessment model is not supported by the live server runtime.",
        status: 400,
        context: {
          layer: "request",
          subsystem: "assessment-runtime",
          operation: "dispatch-assessment-provider-lane",
          modelId: input.requestedModelId,
          canonicalModelId: input.canonicalModelId,
...
export async function generateAssessment(input: {
  ...
  const canonicalModelId = toCanonicalAssessmentModelId(input.request.modelId);
  const model = getModelById(canonicalModelId);
  const providerRuntime = resolveProviderRuntime(model.id);
  const canonicalRequest =
    canonicalModelId === input.request.modelId
      ? input.request
      : {
          ...input.request,
  ...
  const orchestratedPrompt = buildAssessmentPrompt({
    userPrompt: canonicalRequest.prompt,
  ...
  const questions = buildNormalizedAssessmentQuestions({
    ...
    providerModelId: providerRuntime.providerModel,
    ownerUid: input.ownerUid,
    sessionLane: input.sessionLane,
    requestLane: input.requestLane,
  });
