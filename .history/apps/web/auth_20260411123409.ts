...
function handleUnexpectedAuthorizeError(error: unknown, mode: AuthProviderMode): never {
...
async function authorizeUserCredentials(
  ...
  try {
    ...
    handleUnexpectedAuthorizeError(error, "user");
  }
}
...
async function authorizeAdminCredentials(
  ...
  try {
    ...
    handleUnexpectedAuthorizeError(error, "admin");
  }
}
