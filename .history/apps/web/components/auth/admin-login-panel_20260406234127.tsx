...
export function AdminLoginPanel({
  ...
  if (blockingStatus || status?.icon === "permission") {
    supportNotes.push({ text: messages.adminLoginClaimsRunbookNote });
  }
  const buttonLabel =
  ...
  return (
    <div className="relative mx-auto flex w-full max-w-[440px] flex-col gap-3 animate-in fade-in zoom-in-95 duration-700">
      <div className="relative overflow-hidden rounded-[2.1rem] border border-white/15 bg-white/90 p-5 shadow-2xl shadow-black/20 backdrop-blur-2xl dark:bg-zinc-950/72 dark:shadow-black/50 sm:p-6">
        ...
        <div className="relative z-10 flex flex-col gap-4">
          ...
          <div className="flex items-start gap-3">
            <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border border-amber-500/20 bg-amber-500/10 text-amber-700 shadow-sm dark:border-amber-400/20 dark:bg-amber-400/10 dark:text-amber-300">
              <ShieldCheck className="h-5 w-5" aria-hidden="true" />
            </div>
            <div className="min-w-0 space-y-1">
              <h2 className="font-[family-name:var(--font-display)] text-[1.55rem] font-bold tracking-tight text-zinc-900 dark:text-white sm:text-[1.8rem]">
                {messages.adminLoginCardHint}
              </h2>
              <p className="max-w-[28rem] text-xs leading-5 text-zinc-600 dark:text-zinc-400">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 border-t border-zinc-200/70 pt-4 dark:border-zinc-800/80">
            <Link
            ...
            {/* Keep support notes collapsible and conditional so the admin surface stays compact unless runtime help is truly needed. */}
