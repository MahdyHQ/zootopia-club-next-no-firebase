...
export async function DELETE(
  ...
  if (!confirmation) {
    return apiError(
      ...
      "Deletion confirmation is required and must match the target uid or email.",
      400,
    );
  }
  ...
  const matchesEmail =
    ...
    && targetUser.email.trim().length > 0
    && confirmationNormalized === targetUser.email.trim().toLowerCase();

  if (!matchesUid && !matchesEmail) {
    return apiError(
      "DELETE_CONFIRMATION_MISMATCH",
      "Confirmation must match the target user uid or email exactly.",
      400,
    );
  }
  ...
  await appendAdminLog({
    ...
    metadata: {
      ...
      confirmationType: matchesUid ? "uid" : "email",
    },
  });
