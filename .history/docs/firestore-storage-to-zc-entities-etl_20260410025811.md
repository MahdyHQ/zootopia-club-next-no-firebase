...
- `apps/web/lib/server/zootopia-firestore-pg.ts`
...
`owner_uid` derivation must match runtime behavior in `zootopia-firestore-pg.ts`:
...
If removal is later desired, do a dedicated cleanup pass that first updates all docs/comments and deployment runbooks referencing those scripts.
