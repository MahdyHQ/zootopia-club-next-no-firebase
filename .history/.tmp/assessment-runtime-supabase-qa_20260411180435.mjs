...
const PROFILE_PAYLOAD = {
  fullName: "Assessment Qa Test User",
  universityCode: "2601234",
...
function buildCookieHeaderFromSetCookies(setCookies) {
  ...
  for (const setCookie of setCookies) {
    const firstSegment = String(setCookie || "").split(";")[0];
    if (!firstSegment || !firstSegment.includes("=")) {
      continue;
    }

    cookiePairs.push(firstSegment);
  }
...
async function bootstrapAuthJsSessionCookie(baseUrl, providerId, idToken) {
  ...
  const callbackPayload = await callbackResponse.text();
  const callbackSetCookies = readSetCookies(callbackResponse.headers);
  const sessionCookieValue =
    extractCookieValue(callbackSetCookies, "zc_session")
    || extractCookieValue(callbackSetCookies, "authjs.session-token")
    || extractCookieValue(callbackSetCookies, "__Secure-authjs.session-token")
    || extractCookieValue(callbackSetCookies, "next-auth.session-token");

  if (!sessionCookieValue) {
    throw new Error(
      `Missing session cookie after ${providerId} callback (${callbackResponse.status}). Body: ${callbackPayload}`,
    );
  }

  const callbackCookieHeader = buildCookieHeaderFromSetCookies(callbackSetCookies);
  return `${callbackCookieHeader}; zc_theme=light; zc_locale=en`;
}
...
function evaluateProviderPathAttempt(attempt) {
  if ((attempt.status === 200 || attempt.status === 201) && attempt.payload?.ok === true) {
    return {
      pass: true,
...
async function main() {
  ...
  const profilePatchB = await callWithSession({
    ...
    profilePatchA.status === 200,
    `User A profile completion failed (${profilePatchA.status}): ${JSON.stringify(profilePatchA.payload || profilePatchA.text || null)}`,
  );
  ensure(
    profilePatchB.status === 200,
    `User B profile completion failed (${profilePatchB.status}): ${JSON.stringify(profilePatchB.payload || profilePatchB.text || null)}`,
  );
