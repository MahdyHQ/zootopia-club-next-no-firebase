...
## Deprecation Status (2026-04-10)

- `scripts/firebase` is deprecated for long-term operations but retained for current App Hosting secret workflows.
- Do not add new operational responsibilities to these scripts.
- Removal criteria:
	1. A replacement App Hosting secret workflow is documented and approved.
	2. `apps/web/apphosting.yaml` and this runbook are updated to that replacement.
	3. At least one non-interactive secret rotation succeeds using the replacement path.
