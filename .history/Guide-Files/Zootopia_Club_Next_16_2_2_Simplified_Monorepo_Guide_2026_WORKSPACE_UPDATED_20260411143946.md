...
## ⚠️ DEPRECATION NOTICE: Firebase-Era Architecture Reference

This guide describes an EARLIER iteration of the project using Firebase Auth + Firestore + Firebase Storage.

**The current ACTIVE deployment uses:**
- **Supabase Auth + Supabase Postgres + Supabase Storage** (persistence and session)
- **Auth.js** (session management)
- **Vercel** (production deployment)

This guide is retained as a **reference-only** resource for historical context and rollback procedures. **Do NOT use Firebase recommendations from this guide for new work.** Refer instead to the main `zootopia-club-next-ledger.txt` for active truth.
