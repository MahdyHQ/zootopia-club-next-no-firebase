...
- Migration status snapshot against `_migration_history`: `npm run db:status`
- Migration dry run (no writes): `npm run db:apply:dry`
- Migration apply: `npm run db:apply`

`db:status` / `db:apply:*` safely canonicalize legacy renamed migration filenames by timestamp before pending checks, so historical filename cleanup does not trigger SQL re-application.
...
- Server env: `SUPABASE_SERVICE_ROLE_KEY`, `SUPABASE_DATABASE_URL`, `AUTH_SECRET`, `ZOOTOPIA_EMAIL_VERIFICATION_*`, admin email config, and AI provider keys
