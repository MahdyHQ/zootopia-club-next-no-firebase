...
function getFallbackAssessmentExpiry(createdAt) {
  const createdAtTimestamp = Date.parse(String(createdAt || ""));
  if (!Number.isFinite(createdAtTimestamp)) {
    return 0;
  }

  return createdAtTimestamp + ASSESSMENT_RETENTION_WINDOW_MS;
}

function isPreviewableAssessmentRecord(record) {
  if (!record || record.status === "expired") {
    return false;
  }

  const explicitExpiryTimestamp = Date.parse(String(record.expiresAt || ""));
  const effectiveExpiryTimestamp = Number.isFinite(explicitExpiryTimestamp)
    ? explicitExpiryTimestamp
    : getFallbackAssessmentExpiry(record.createdAt);

  return Number.isFinite(effectiveExpiryTimestamp) && effectiveExpiryTimestamp > Date.now();
}

async function discoverAssessmentRoutesFromFirestore(firebaseApp, ownerUid) {
  const snapshot = await getFirestore(firebaseApp, FIRESTORE_DATABASE_ID)
  ...
  const candidate = snapshot.docs.find((doc) => {
    const data = doc.data();
    return isPreviewableAssessmentRecord(data);
  });
  ...
  const assessmentId = candidate.id;
  return {
    preview: `/assessment/preview/${assessmentId}`,
    result: `/assessment/results/${assessmentId}`,
  };
}

async function createAssessmentRoutesFromApi(baseUrl, sessionCookie) {
  const response = await fetch(`${baseUrl}/api/assessment`, {
    method: "POST",
    headers: {
      "content-type": "application/json",
      cookie: `zc_session=${sessionCookie}; zc_theme=light`,
    },
    body: JSON.stringify({
      prompt:
        "Generate a concise biology assessment about animal cell organelles and their core functions.",
      options: {
  ...
  if (!response.ok || payload?.ok !== true) {
    console.error("API generation fallback failed", {
  ...
  const generatedId = payload?.data?.generation?.id;
  if (typeof generatedId !== "string" || generatedId.trim().length === 0) {
    console.error("API generation fallback returned an invalid generation ID", {
      payload,
    });
    return null;
  }
  ...
  return {
    preview: `/assessment/preview/${generatedId}`,
    result: `/assessment/results/${generatedId}`,
  };
}

async function getRuntimeFlags(baseUrl) {
  try {
    ...
    const payload = await response.json().catch(() => ({}));
    const runtimeFlags = payload?.data?.runtimeFlags;
...
async function main() {
  ...
  const runtimeFlags = await getRuntimeFlags(baseUrl);
  const canUseFirestoreFallback = runtimeFlags?.firebaseAdmin === true;
  ...
  const combinations = [];
  let routeDiscoverySource = "history";
  ...
  try {
    ...
    try {
      ...
      try {
        discoveredPaths = await discoverAssessmentRoutes(discoveryPage, baseUrl);
      } catch {
        if (canUseFirestoreFallback) {
          discoveredPaths = await discoverAssessmentRoutesFromFirestore(
            firebaseApp,
            selectedAdmin.uid,
          );
          if (discoveredPaths) {
            routeDiscoverySource = "firestore-fallback";
          }
        }
      }

      if (!discoveredPaths) {
        discoveredPaths = await createAssessmentRoutesFromApi(baseUrl, sessionCookie);
        if (discoveredPaths) {
          routeDiscoverySource = "generated-fallback";
        }
      }

      if (!discoveredPaths) {
        throw new Error(
          "Unable to discover preview/result routes from History, Firestore fallback, or API generation fallback.",
        );
      }
