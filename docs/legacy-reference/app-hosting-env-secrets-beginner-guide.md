...
- Use legacy App Hosting YAML contracts only as rollback/reference material.
- Keep real secret values in Cloud Secret Manager only.
- Treat ZOOTOPIA_ADMIN_LOGIN_PASSWORD as legacy/deprecated context only (not part of active admin auth).
...
| Admin password gate (legacy/deprecated) | ZOOTOPIA_ADMIN_LOGIN_PASSWORD | secret | optional legacy rollback only | optional | apphosting secret reference (RUNTIME) |
...
This repo no longer uses ZOOTOPIA_ADMIN_LOGIN_PASSWORD in the active admin auth flow.
If kept for legacy rollback, it must remain an optional server-only factor and never the sole authority.

Why active admin auth remains safe:

- Admin auth is enforced by Supabase sign-in plus server-side allowlist and role/claim checks.
- Session issuance requires verified Supabase access tokens, recency checks, allowlist checks, and claim/status checks.

Unsafe patterns to avoid:

- Do not reintroduce shared-password-only admin checks.
