...
export async function POST(request: Request) {
  ...
  let body: { idToken?: string; adminLoginPassword?: string };

  try {
    body = (await request.json()) as { idToken?: string; adminLoginPassword?: string };
  } catch {
  ...
  const adminPasswordGate = verifyAdminLoginPasswordGate(body.adminLoginPassword);
  if (!adminPasswordGate.ok) {
    return applyNoStore(apiError(
      adminPasswordGate.code,
      adminPasswordGate.message,
      adminPasswordGate.status,
    ));
  }
