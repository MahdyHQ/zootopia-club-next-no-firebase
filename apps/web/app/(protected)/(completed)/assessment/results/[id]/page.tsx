...
function renderUnavailableCard(input: {
  ...
  return (
    <section className="rounded-[2rem] border border-white/10 bg-background-elevated/84 p-8 shadow-sm">
...
export default async function AssessmentResultPage(props: {
  ...
  try {
    ...
    console.warn("[assessment-result-page] failed to load generation; rendering unavailable state", {
      assessmentId: id,
      uid: user.uid,
  ...
  try {
    ...
    console.warn("[assessment-result-page] failed to build preview model; rendering unavailable state", {
