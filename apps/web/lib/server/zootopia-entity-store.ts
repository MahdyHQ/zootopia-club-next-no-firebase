...
function readDatabaseUrl() {
  const raw =
    /* Active repo env contract treats DATABASE_URL as canonical for Postgres connectivity.
       Keep SUPABASE_DATABASE_URL as a compatibility alias so existing local files and old
       rollback notes do not break while docs and environments finish converging. */
...
export function getZootopiaEntityStore() {
  return new PgEntityStore(getZootopiaSql());
}
