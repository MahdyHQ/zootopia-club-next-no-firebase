...
/* This file owns the Fast browser-print HTML surface that users save to PDF from the browser.
   ...
   print-safe tweaks stay lane-owned instead of drifting back into a hidden shared renderer. */

/* Fast HTML artifacts must invalidate when the Fast browser-print surface changes materially.
   Keep this version lane-specific so renderer-content migration and later Fast-only print work
   do not silently force Pro PDF artifact churn. */
export const ASSESSMENT_FAST_PRINT_RENDERER_VERSION = "fast-2026-04-14-renderer-content-migration-v2";

export type AssessmentFastPrintRenderDiagnostics = {
  bodyLoaded: boolean;
...
export function buildAssessmentFastPrintRenderDiagnostics(input: {
  preview: NormalizedAssessmentPreview;
  html?: string | null;
}): AssessmentFastPrintRenderDiagnostics {
  ...
  /* Export diagnostics intentionally stay shape-level instead of logging question text. This lets
     the PDF lanes prove that real assessment body blocks reached the Fast lane renderer without
...
function renderChoiceItem(input: {
  ...
  /* The exported answer badge must stay deterministic across both browser-print and Chromium
     capture surfaces. Keep the checkmark as inline SVG markup so lightweight glyph fallback
     cannot disappear when either lane renders the former shared design in a different engine. */
...
export function buildAssessmentFastPrintHtml(input: {
  preview: NormalizedAssessmentPreview;
  ...
  /* The Fast renderer's primary owner is the authenticated browser-print route, so keep
     auto-print enabled by default and avoid a Pro-style absolute-base-url contract here.
     Future agents should add Fast-specific inputs only when the in-browser print lane truly
     needs them, not by copying Pro capture concerns back into this renderer. */
  const {
    preview,
    themeMode,
    qrCodeDataUrl,
  ...
  /* Keep the summary chip label literal and shared with the detached preview/result shell so the
     premium file header reads as one consistent SUMMARY surface across both export lanes. */
